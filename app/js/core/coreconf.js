...
import { ensureYangMaps } from '../utils/mapping.js';
...
export class CoreconfClient{
  ...
  async _send(method, uri, payload){ if(!this.state.transport.writer) throw new Error('not connected'); this._wire(); ensureYangMaps(); const yp=uri.replace(/^\//,''); const sid=window.yangToSidMap?.[yp]; const target=sid? `/${sid}`: uri; await this.state.transport.writer.write(new TextEncoder().encode(this.mup1.createCoapInitFrame())); const msg=this.coap.buildMessage(method,target,payload); const frame=this.mup1.createCoapDataFrame(msg); await this.state.transport.writer.write(new TextEncoder().encode(frame)); const id=this.coap.messageId-1; return new Promise((res,rej)=>{ const to=setTimeout(()=>{ this.pending.delete(id); rej(new Error('timeout')); },10000); this.pending.set(id,{res,rej,to}); }); }
